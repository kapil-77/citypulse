  ...
  return (
    <div className="h-full flex flex-col bg-[var(--bg-page)]">
      <TopBar title="Issue" showBack rightAction={
        <button onClick={handleShare} className="w-7 h-7 flex items-center justify-center hover:bg-white/10 transition-colors" aria-label="Share">
        ...
        <div className="container py-8 space-y-8">
          ...
          <Card>
            <div className="border-b border-[var(--border-light)] px-5 py-3">
              <p className="label text-[0.625rem]">Location</p>
            </div>
            <div className="p-5">
              <p className="text-sm font-medium text-[var(--text-primary)]">{issue.address}</p>
              <p className="text-xs text-[var(--text-muted)] mt-1">{issue.location.lat.toFixed(6)}, {issue.location.lng.toFixed(6)}</p>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 border border-[var(--black)] bg-[var(--bg-muted)] flex items-center justify-center text-sm font-medium">{issue.reportedBy?.name?.[0] || 'A'}</div>
              <div className="flex-1">
                <p className="text-sm font-medium">{issue.reportedBy?.name || 'Anonymous'}</p>
                <p className="text-xs text-[var(--text-muted)]">{formatDate(issue.reportedAt)}</p>
              </div>
              <span className="label text-[0.625rem]">{formatRel(issue.reportedAt)}</span>
            </div>
            {issue.resolvedAt && <div className="mt-3 pt-3 border-t border-[var(--border-light)]"><span className="text-sm text-[var(--status-green)]">Resolved {formatDate(issue.resolvedAt)}</span></div>}
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-0">
              {[
                { label: 'Reported', icon: 'R', color: 'bg-[var(--black)]', time: formatRel(issue.reportedAt), show: true },
                { label: 'Under Review', icon: '→', color: 'bg-[var(--black)]', show: ['under_review', 'work_started', 'in_progress', 'resolved', 'verified_resolved'].includes(issue.status) },
                { label: 'Work Started', icon: 'W', color: 'bg-[var(--black)]', show: ['work_started', 'in_progress', 'resolved', 'verified_resolved'].includes(issue.status) },
                { label: 'In Progress', icon: '→', color: 'bg-[var(--black)]', show: ['in_progress', 'resolved', 'verified_resolved'].includes(issue.status) },
                { label: 'Resolved', icon: '✓', color: 'bg-[var(--black)]', show: ['resolved', 'verified_resolved'].includes(issue.status) },
                ...
                <div key={step.label} className="flex gap-3 pb-4 relative">
                  ...
                  <div className={`relative z-10 flex-shrink-0 w-6 h-6 ${step.color} flex items-center justify-center`}>
          ...
          {issue.timeline && issue.timeline.length > 0 && (
            <Card><CardHeader><CardTitle>Activity</CardTitle></CardHeader><TimelineFeed events={issue.timeline} /></Card>
          )}

          {relatedIssues.length > 0 && (
            <Card>
              <CardHeader><CardTitle>Related Issues</CardTitle><span className="label text-[0.625rem]">{relatedIssues.length} similar</span></CardHeader>
              <div className="divide-y divide-[var(--border-light)]">{relatedIssues.map((r) => <IssueCard key={r.id} issue={r} compact />)}</div>
            </Card>
          )}
